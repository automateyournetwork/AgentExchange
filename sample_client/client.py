...
AGENT_REGISTRY_URL = os.getenv("AGENT_REGISTRY_URL", "https://32bb-76-68-137-181.ngrok-free.app/agents")
...
async def fetch_agents_from_registry(id_token: str) -> list[dict]:
    headers = {"Authorization": f"Bearer {id_token}"}
    async with httpx.AsyncClient(headers=headers) as client:
        response = await client.get(AGENT_REGISTRY_URL)
        response.raise_for_status()
        return response.json().get("agents", [])
    
def choose_agent(agents: list[dict]) -> dict:
    print("\n📋 Registered Agents:")
    for i, agent in enumerate(agents):
        print(f"{i+1}. {agent['name']} ({agent['endpoint']})")
    index = int(input("🔢 Choose an agent by number: ")) - 1
    return agents[index]
...
async def main():
    ...
    try:
        ...
        async with httpx.AsyncClient(headers=headers, timeout=600.0) as http_client_session:
            agents = await fetch_agents_from_registry(id_token_str)
            selected_agent = choose_agent(agents)  # ✅ Move this here first

            agent_card_full_url = urljoin(selected_agent["endpoint"], ".well-known/agent.json")
